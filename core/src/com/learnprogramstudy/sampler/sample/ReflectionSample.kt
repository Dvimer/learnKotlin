...
class ReflectionSample : SampleBase() {
    ...
    //    private fun <T : Any> debugReflection(clazz: Class<T>) {}
    private inline fun <reified T : Any> debugReflection() {
        var fields = ClassReflection.getDeclaredFields(T::class.java)
        var methods = ClassReflection.getDeclaredMethods(T::class.java)

        log.debug("Reflection class ${T::class.java.simpleName}")
        log.debug("Field count = ${fields.size}")

        for (field in fields) {
            log.debug("name field = ${field.name}, field type = ${field.type}")
        }

        log.debug("Methods count = ${methods.size}")

        methods.forEach {
            log.debug("method name = ${it.name}, type = ${it.parameterTypes}, parametrSize = ${it.parameterTypes.size}")
        }
    }
}
